...
def main():
  ...
  showSteps = True
  showWarnings = False
  saveFileName = "results-bottles-mc-{}.mat".format(randomSeed)
  params = locals()
  ...
  for episode in xrange(randomSeed, nEpisodes):
    ...
    for t in xrange(nObjects):
      ...
      # segment objects
      startTime = time()
      freeBottles, freeBottleProbs, placedBottles, freeCoasters, occupiedCoasters, segmentationDist\
      ...
      completionSamples, completionSampleNormals = regraspPlanner.SampleShapeCompletions(cloud,
        freeBottles, segmentationDist, modelCompletion, completedFreeBottles, completionProbs,
      ...
      if showSteps:
        for i in xrange(nCompletionSamples):
          sample = [completionSamples[j][i] for j in xrange(len(freeBottles))]
          objectId = [(1 + j) * ones(nOutputPointsCompletion) / len(freeBottles) for \
            j in xrange(len(freeBottles))]
      ...
      startTime = time()
      pickPlaces, plannedConfigs, goalIdx, targObjIdx = regraspPlanner.Plan(completedFreeBottles,
