...
def sample_and_group(npoint, radius, nsample, xyz, points, knn=False, use_xyz=True):
  ...
  #new_xyz = tf.Print(new_xyz, [new_xyz], message="new_xyz", summarize=200)
  if knn:
...
def pointnet_sa_module(xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training,
  ...
  with tf.variable_scope(scope):
    ...
    for i, num_out_channel in enumerate(mlp):
      ####################################
      new_points = Ops.xxlu(Ops.conv2d(new_points, k = (1, 1), out_c = num_out_channel, str = 1,
        pad = 'VALID', name = 'l' + str(i)), label = 'lrelu')
    ...
    if mlp2 is not None:
      ...
      for i, num_out_channel in enumerate(mlp2):
        ####################################
        new_points = Ops.xxlu(Ops.conv2d(new_points, k=(1, 1), out_c=num_out_channel, str=1,
          pad='VALID', name='ll' + str(i)), label='lrelu')
...
def pointnet_sa_module_msg(xyz, points, npoint, radius_list, nsample_list, mlp_list, is_training,
  bn_decay, scope, bn=True, use_xyz=True, use_nchw=False):
  ...
  with tf.variable_scope(scope):
    ...
    for i in range(len(radius_list)):
      ...
      for j,num_out_channel in enumerate(mlp_list[i]):
        ####################################
        grouped_points = Ops.xxlu(Ops.conv2d(grouped_points, k = (1, 1), out_c = num_out_channel,
          str = 1, pad = 'VALID', name = 'lll' + str(i)), label = 'lrelu')
...
def pointnet_fp_module(xyz1, xyz2, points1, points2, mlp, is_training, bn_decay, scope, bn=True):
  ...
  with tf.variable_scope(scope):
    ...
    for i, num_out_channel in enumerate(mlp):
      ####################################
      new_points1 = Ops.xxlu(Ops.conv2d(new_points1, k = (1, 1), out_c = num_out_channel, str = 1,
        pad = 'VALID', name = 'llll' + str(i)), label = 'lrelu')
