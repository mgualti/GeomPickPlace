...
from numpy import arccos, argmin, arange, array, ascontiguousarray, concatenate, cos, cross, dot, \
  eye, exp, hstack, isnan, log, logical_and, logical_not, max, mean, minimum, nonzero, ones, pi, \
  power, repeat, reshape, sum, tile, unique, vstack, where, zeros
# openrave
import openravepy
# self
import point_cloud
...
class PlannerRegrasp(Planner):
  ...
  def __init__(self, env, temporaryPlacePosition, nGraspSamples, halfAngleOfGraspFrictionCone,
    ...
    insignificantCost, maxSearchDepth, maxIterations, minPointsPerSegment):
  ...
  def GetPlannedArmConfigs(self, pickPlaces, obstacleCloudAtStart, obstacleCloudAtGoal):
    '''Finds feasible 6-DoF configurations of the arm for a pick-place plan as found by PlanWorker.
    
    - Input pickPlaces: List of 4x4 numpy matrices, homogeneous transforms for the gripper in the
      world frame, one for each step of the pick-place plan.
    - Input obstacleCloudAtStart: Point cloud (nx3 numpy array) indicating the obstacles at the
      object's initial location.
    - Input obstacleCloudAtGoal: Point cloud (nx3 numpy array) indicating the obstacles at the
      object's goal location.
    - Returns plannedConfigs: A 6-DoF arm configuration for each element in pickPlaces that is
      collision-free and within the joint limits.
    '''
    ...
    if len(pickPlaces) % 2 != 0:
      raise Exception("Pick-place plan does not have an even number of steps.")
      
    if obstacleCloudAtStart.shape[1] != 3 or obstacleCloudAtGoal.shape[1] != 3:
      raise Exception("An obstacle cloud is not 3D.")
      
    # find closest reachable configuration for start placement
    ik, reachable = self.env.GetReachableConfigs([pickPlaces[0]], True, obstacleCloudAtStart)
    plannedConfigs = [self.env.ClosestIkToGiven(ik[0], self.env.GetHomeConfig())]
    
    # find closest reachable configuration for temporary placements
    ik, reachable = self.env.GetReachableConfigs(pickPlaces[1:-1], True)
    for solutions in ik:
      plannedConfigs.append(self.env.ClosestIkToGiven(solutions, plannedConfigs[-1]))
    
    # find closest reachable configuration for goal placement
    ik, reachable = self.env.GetReachableConfigs([pickPlaces[-1]], True, obstacleCloudAtGoal)
    plannedConfigs.append(self.env.ClosestIkToGiven(ik[0], plannedConfigs[-1]))
  ...
  def Plan(self, segmClouds, segmCorrectProbs, compClouds, compCorrectProbs, normals, goalPoses,
    ...
    - Returns configs: (Closest from home and previous) reachable configuration at each pick/place.
    ...
    # wait for result
    pickPlaces = []; configs = []; goalIdxs = []; costs = []; workerIdxs = []
    ...
    while not done.all():
      ...
      if connections[i].poll(0.1):        
        ...
        if purpose == "regrasp":
          ...
          configs.append(data[1])
          goalIdxs.append(data[2])
          costs.append(data[3])
          workerIdxs.append(i)
          done[i] = True
          # the lower bound for this worker is no longer achievable
          bestPossibleCosts[i] = data[3] + self.insignificantCost
    ...
    return pickPlaces[bestIdx], configs[bestIdx], goalIdx, targObjIdx
  ...
  def UpdateGraspPlaceTable(self, graspPlaceTable, grasps, graspsNew, graspCosts, graspCostsNew,
    ...
    reachableStart = self.env.CheckReachability(posesStart, True, obstacleCloudAtStart)
    ...
    reachableGoal = self.env.CheckReachability(posesGoal, True, obstacleCloudAtGoal)
    ...
    reachableTemp = self.env.CheckReachability(posesTemp, True)
