...
from numpy import arccos, argmax, argmin, arange, array, copy, exp, isnan, log, max, min, minimum, \
  mean, ones, pi, power, logical_and, repeat, reshape, sqrt, sum, tile, unique, where, zeros
# self
import point_cloud
...
class PlannerRegraspMC(PlannerRegrasp):
  ...
  def GetGraspCosts(self, cloud, normals, grasps, pointPairs, sampleClouds, sampleNormals):
    ...
    - Input grasps: List of m grasps (4x4 homogeneous transforms) to evaluate the cost for.
    ...
    - Returns graspCosts: m-element array, negative log (estimated) probability of grasp success.
    '''
    ...
    if len(grasps) != len(pointPairs):
      raise Exception("Length of grasps, {}, must be the same as the length of pointPairs, {}.".\
        format(len(grasps), len(pointPairs)))
    ...
    if self.graspCostFactor > 0:
      ...
      mcCost = 0.5 * self.graspCostFactor * log((1.0 + len(sampleClouds)) / nFeasibleShapes)
    
    else:
      mcCost = zeros(len(grasps))
    
    # Compute antipodal cost    
    ...
    if self.antipodalCostFactor > 0:
      ...
      antipodalCost = zeros(len(grasps))
    
    return mcCost + antipodalCost
  ...
  def PlanWorker(self, cloud, normals, cloudSamples, normalSamples, goalPlaces, goalCosts,
    ...
    for iteration in xrange(self.maxIterations):
      ...
      if notFirstLoop:
        ...
        graspCostsNew = self.GetGraspCosts(
          cloud, normals, graspsNew, pairsNew, cloudSamples, normalSamples)
      else:
        notFirstLoop = True
  ...
  def PreSampleGraspsOnObjects(self, clouds, normals, cloudSamples, normalSamples, nGraspSamples,
    ...
    for i in xrange(len(clouds)):
      ...
      graspCosts = self.GetGraspCosts(
        clouds[i], normals[i], grasps, pairsForGrasps, cloudSamples[i], normalSamples[i])
  ...
  def SampleShapeCompletions(self, inputCloud, segmentedClouds, segmentationDist, completionModel,
    ...
    for i in xrange(nCompletionSamples):    
      ...
      if self.segmUncertCostFactor > 0:
        ...
        sampleSegmentation = deepcopy(segmentedClouds)
      ...
      if self.segmUncertCostFactor > 0:
        ...
        sampleCompletion = deepcopy(completedClouds)
