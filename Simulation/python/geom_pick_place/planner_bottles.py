...
from numpy import argmax, argmin, argsort, array, copy, cov, cross, dot, eye, mean, ones, pi, \
...
class PlannerBottles(Planner):
  ...
  def __init__(self, env, nGoalOrientations, minPointsPerSegment):
    '''TODO'''
  ...
  def SegmentObjects(self, cloud, modelSegmentation, scoreThresh, useGroundTruth, nObjects):
    '''TODO'''
    
    # perform segmentation
    
    segmentedClouds, segmentationProbs, segmentationDist = super(PlannerBottles, self).\
      SegmentObjects(cloud, modelSegmentation, scoreThresh, useGroundTruth)
      
    if len(segmentedClouds) < nObjects:
      print("Less than {} objects segmented: nothing to place.".format(nObjects))
      return [], [], [], [], [], segmentationDist
