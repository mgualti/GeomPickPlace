...
# geom_pick_place
from place_packing_params_mc import Parameters
...
def main():
  ...
  certainEquivSegmProb = 0.70 # 0.80 in simulation
  ...
  regraspPlanner = PlannerRegraspMC(env, regraspPosition, nGraspSamples,
  ...
  while not rospy.is_shutdown():
    ...
    completionSamples, completionSampleNormals = regraspPlanner.SampleShapeCompletions(
      cloud, segmentedClouds, segmentationDist, modelCompletion, completedClouds, completionProbs,
    ...
    goalPoses, goalCosts, targObjIdxs = taskPlanner.GetGoalPoses(completedClouds, normals,
      completionSamples, completionSampleNormals, boxCloud, regraspPlanner)
    ...
    pickPlaces, goalIdx, targObjIdx = regraspPlanner.Plan(completedClouds, normals,
      completionSamples, completionSampleNormals, goalPoses, goalCosts, targObjIdxs,
    ...
    if showSteps and len(pickPlaces) > 0:
    ...
    for motionPlanningAttempt in xrange(nMotionPlanningAttempts):
      ...
      if len(pickPlaces) > 0:
        ...
        homeToPreGraspTraj, preGraspToHomeTraj, graspConfig, homeToPrePlaceTraj, \
          prePlaceToHomeTraj, placeConfig = utilities.PlanPickPlace(pickPlaces[0], pickPlaces[1],
          completedClouds[targObjIdx], obstacleCloudsAtStart[goalIdx],
          obstacleCloudsAtGoal[goalIdx], showSteps)
    ...
    if len(pickPlaces) == 0 or homeToPreGraspTraj is None:
