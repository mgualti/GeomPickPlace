...
# python
# scipy
from numpy import argsort, concatenate, dot, sqrt, square
# ros
import rospy
# self
import point_cloud
...
class UtilitiesPacking(Utilities):
  ...
  def ExecutePickPlace(self, homeToPreGraspTraj, preGraspToHomeTraj, graspConfig, \
    ...
    # home -> preGrasp
    self.arm.FollowTrajectory(homeToPreGraspTraj, gain = 0.65, gamma = 0.93)
    rospy.sleep(1.0)
    
    # preGrasp -> grasp
    self.arm.FollowTrajectory([graspConfig], gain = 0.65, gamma = 0.93, maxDistToTarget = 0.01)
    rospy.sleep(1.0)
    ...
    # grasp -> preGrasp
    self.arm.FollowTrajectory(homeToPreGraspTraj[-1:], gain = 0.65, gamma = 0.93)
    rospy.sleep(1.0)
    
    # preGrasp -> home
    self.arm.FollowTrajectory(preGraspToHomeTraj, gain = 0.65, gamma = 0.93)
    rospy.sleep(1.0)
    ...
    # home -> prePlace
    self.arm.FollowTrajectory(homeToPrePlaceTraj, gain = 0.65, gamma = 0.93)
    rospy.sleep(1.0)  
    
    # prePlace -> place
    self.arm.FollowTrajectory([placeConfig], gain = 0.65, gamma = 0.93, maxDistToTarget = 0.01,
      forceInterrupt = 3000)
    rospy.sleep(1.0)
    ...
    # place -> prePlace
    self.arm.FollowTrajectory(homeToPrePlaceTraj[-1:], gain = 0.65, gamma = 0.93)
    rospy.sleep(1.0)   
    
    # prePlace -> home
    self.arm.FollowTrajectory(prePlaceToHomeTraj, gain = 0.65, gamma = 0.93)
    rospy.sleep(1.0)   
  ...
  def PlanPickPlace(self, grasp, place, objectCloud, obstacleCloudAtStart, obstacleCloudAtGoal, showSteps):
    '''TODO'''
    
    # compute grasp IKs
    preGrasp = self.regraspPlanner.GetPreGrasps([grasp])[0]
    preGraspConfig, graspConfig = self.GetConfigs(
      preGrasp, grasp, objectCloud, obstacleCloudAtStart, None)
    ...
    # compute place IKs
    prePlace = self.regraspPlanner.GetPreGrasps([place])[0]
    prePlaceConfig, placeConfig = self.GetConfigs(
      prePlace, place, objectCloud, obstacleCloudAtGoal, graspConfig)
    ...
    # plan home -> preGrasp
    self.env.AddObstacleCloud(concatenate([objectCloud, obstacleCloudAtStart]), 0.01)
    ...
    # plan preGrasp -> home
    self.env.AddObstacleCloud(obstacleCloudAtStart, 0.01)
    ...
    '''if showSteps:
      self.VisualizeTrajectory(preGraspToHomeTraj, "preGrasp->home")'''
      
    # plan home -> prePlace
    self.env.AddObstacleCloud(obstacleCloudAtGoal, 0.01)
    ...
    # plan prePlace -> home
    objectCloudAfterPlacement = point_cloud.Transform(
      dot(place, point_cloud.InverseTransform(grasp)), objectCloud)
    self.env.AddObstacleCloud(concatenate([objectCloudAfterPlacement, obstacleCloudAtGoal]), 0.01)
