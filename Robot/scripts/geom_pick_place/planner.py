...
class Planner(object):
  ...
  def __init__(self, env, minPointsPerSegment, preGraspOffset):
    ...
    - Input preGraspOffset: To increase likelihood a motion plan will be found, "pre-grasps", backed
      up along the approach direction from the original grasp, are checked for reachability. This
      offset is the distance in meters of the pregrasp from the original grasp in the negative
      approach direction.
    '''
    ...
    if not isinstance(preGraspOffset, (int, long, float)):
      raise Exception("Expected preGraspOffset to be a scalar.")
    
    # assignments
    self.env = env
    self.minPointsPerSegment = minPointsPerSegment
    self.preGraspOffset = preGraspOffset
  ...
  def GetPreGrasps(self, grasps):
    '''TODO'''
    
    preGrasps = []
    for grasp in grasps:
      preGrasp = copy(grasp)
      preGrasp[0:3, 3] += self.preGraspOffset * preGrasp[0:3, 2]
  ...
  def SegmentObjects(self, cloud, model, scoreThresh, useGroundTruth):
    ...
    if useGroundTruth:
      ...
      distribution = copy(segmentation)
    else:
