...
from numpy import abs, arange, argmax, argmin, array, ascontiguousarray, cross, dot, empty, eye, \
  hstack, logical_and, logical_not, max, mean, nonzero, power, repeat, reshape, sort, sum, tile, zeros
# openrave
import openravepy
# self
import point_cloud
...
class EnvironmentPickPlace(Environment):
  ...
  def AreAnyReachable(self, grasps, preGrasps, ignoreObjects = False):
    '''TODO'''
    ...
    if len(grasps) != len(preGrasps):
      raise Exception("Length of grasp and pre-grasp list is not the same.")
    ...
    for i in xrange(len(grasps)):
      reachable[i] = not self.IsHandUnderTable(grasps[i]) and not self.IsHandUnderTable(preGrasps[i])
    
    # check grasps for IK solutions
    configs = [[]] * len(grasps)
    ...
    # check pre-grasps for IK solutions
    preConfigs = [[]] * len(preGrasps)
    for i, T in enumerate(preGrasps):
      if not reachable[i]: continue
      preConfigs[i] = self.CalcIkForT(T)
      reachable[i] = len(preConfigs[i]) > 0
  ...
  def CheckReachability(self, grasps, preGrasps, ignoreObjects = False, obstacleCloud = None,
    cubeSize = 0.01):
    '''TODO'''
    ...
    # check for pre-grasp IK solutions
    preConfigs = [[]] * len(preGrasps)
    for i, T in enumerate(preGrasps):
      if not reachable[i]: continue
      preConfigs[i] = self.CalcIkForT(T)
    ...
    # check for pre-grasp collisions
    for i, T in enumerate(preGrasps):
      if not reachable[i]: continue
      reachable[i] = False
      for config in preConfigs[i]:
        self.MoveRobot(config)
        if not self.env.CheckCollision(self.robot):
          reachable[i] = True
          break
